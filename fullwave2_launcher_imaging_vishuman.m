...
for k=3:7
  idl=circleIdx(size(scats),[nXextend/2 k*1e-2/dX],3e-3/dY);
  scats(idl)=0; % anechoic region
end
...
figure(2)
n=1:tx.nTx; bws=((n-(tx.nTx+1)/2)*tx.bmw);
imagesc(bws*1e3,deps*1e3,dbzero(abs(hilbert(squeeze(bm1)))),[-40 0])
colormap gray, colorbar
xlabel('mm'), ylabel('mm')
axis equal, axis tight
...
figure(3)
n=1:tx.nTx; bws=((n-(tx.nTx+1)/2)*tx.bmw);
imagesc(bws*1e3,deps*1e3,dbzero(abs(hilbert(squeeze(bm2)))),[-40 0])
colormap gray, colorbar
xlabel('mm'), ylabel('mm')
axis equal, axis tight

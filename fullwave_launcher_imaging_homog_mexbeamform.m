  ...
  if(n==1)
      ...
      for ii=1:length(lats)
          ...
          for jj=1:length(deps)
              dep=deps(jj);
              fcen=([lat/dY+mean(xducercoords(:,1)) dep/dY]);
...
for ii=1:length(lats)
    for jj=1:length(deps)
        bm(ii,jj,n)=sum(pxducer(idps_mat(ii,jj,2:idps_mat(ii,jj,1))));
    end
end
toc

pxducer2=pxducer; pxducer2(:,:,2)=pxducer; % the third dimension of pxducer is the frame, this function is meant for multiple frames, so we are simply copying a frame 

%% mex beamforming using an OMP parellelized mex function %%
tic
tmp=mex_beamform_omp4_float(int64(idps_mat),single(pxducer2))';
toc
...
figure(2)
imagesc(bws*1e3,deps*1e3,dbzero(abs(hilbert(squeeze(bmm)))),[-40 0])
colormap gray
xlabel('mm'), ylabel('mm')
axis equal, axis tight
